...
/**
 * Sleep for a given number of milliseconds without busy-waiting.
 * Uses Atomics.wait() which actually yields CPU instead of spin-looping.
 *
...
function isDaemonReachable(projectDir) {
    ...
    if (connInfo.type === 'tcp') {
        ...
        try {
            ...
            // Give it a moment - use actual sleep instead of busy-wait
            sleepMs(200);
            return connected;
        }
        catch {
    ...
    try {
        ...
        if (!tryAcquireLock(projectDir)) {
            ...
            while (Date.now() - start < 5000) {
                ...
                // Brief wait - use actual sleep instead of busy-wait
                sleepMs(100);
            }
 ...
 */
export async function extractDaemon(filePath, projectDir, sessionId) {
 ...
 */
export async function treeDaemon(projectDir, extensions, excludeHidden = true) {
    const response = await queryDaemon({ cmd: 'tree', extensions, exclude_hidden: excludeHidden }, projectDir);
    return response.result || response;
}
/**
 ...
 */
export async function structureDaemon(projectDir, language = 'python', maxResults = 100) {
    const response = await queryDaemon({ cmd: 'structure', language, max_results: maxResults }, projectDir);
    return response.result || response;
}
/**
 ...
 */
export async function contextDaemon(projectDir, entry, language = 'python', depth = 2) {
    const response = await queryDaemon({ cmd: 'context', entry, language, depth }, projectDir);
    return response.result || response;
}
/**
 ...
 */
export async function importsDaemon(projectDir, filePath, language = 'python') {
    const response = await queryDaemon({ cmd: 'imports', file: filePath, language }, projectDir);
 ...
 */
export async function importersDaemon(projectDir, module, language = 'python') {
 ...
 */
export function trackHookActivity(hookName, projectDir, success = true, metrics = {}) {
    // Fire-and-forget - don't await, don't block
    queryDaemon({ cmd: 'track', hook: hookName, success, metrics }, projectDir).catch(() => {
 ...
 * @param metrics - Hook-specific metrics to track
 */
export function trackHookActivitySync(hookName, projectDir, success = true, metrics = {}) {
    try {
        queryDaemonSync({ cmd: 'track', hook: hookName, success, metrics }, projectDir);
    }
    catch {
        // Silently ignore errors - stats tracking is best-effort
    }
