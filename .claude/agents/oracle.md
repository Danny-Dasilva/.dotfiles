...
1. **Fetch provided URLs FIRST** - If the task includes specific URLs, fetch them directly using WebFetch BEFORE doing web searches. Web search may not have indexed new content (repos created in the last few days/weeks won't appear in search results). The provided URLs are the ground truth.
2. **Cite sources** - every claim needs a reference
3. **Verify currency** - check publication dates
4. **Cross-reference** - don't trust single sources
5. **State confidence** - be honest about uncertainty
6. **Extract actionable info** - not just links
7. **Check official docs first** - then community sources
8. **Write to output file** - don't just return text
9. **Never claim something doesn't exist** without first fetching any URLs the user provided. New repos/models may not be indexed by search engines yet.
